...
func Fingerprint(libID int64, fragLen int, c2t5p, g2a3p, meanErr float64) string {
	identity := fmt.Sprintf("lib=%d|len=%d|c2t=%s|g2a=%s|err=%s",
		libID, fragLen, strconv.FormatFloat(c2t5p, 'g', -1, 64),
		strconv.FormatFloat(g2a3p, 'g', -1, 64), strconv.FormatFloat(meanErr, 'g', -1, 64))
